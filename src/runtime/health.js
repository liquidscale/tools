...
export default function (runtime) {
  const log = runtime.logger.child({ internal: true, module: "health", level: process.env.NODE_ENV !== "production" ? "info" : "warn" });
  ...
  runtime.events.pipe(filter(event => event.key.indexOf("component:installed:") === 0)).subscribe(async ({ key, component }) => {
    ...
    if (component.stereotype === "dataset" && component.lang === "yaml") {
      ...
      if (dataset.type === "snapshot") {
        log.debug("injecting test dataset %s in scope %s at height %d", component.key, dataset.scope, dataset.height);
        await targetScope.store.injectSnapshot({ height: dataset.height, data: dataset.data, ts: dataset.ts });
      }
    } else {
      if (key.indexOf(":updated") !== -1) {
        log.debug("%s %s has been changed. Let's execute all associated tests".gray.dim, component.stereotype, component.key);
      } else {
        log.debug("running initial health checks for %s %s".gray.dim, component.stereotype, component.key);
      }
    }
  });
